...
import { LayoutDashboard, Building2, Network, Package, MapPin, FileText, Users, Tag, FileEdit, BarChart3, Settings, ShieldCheck, LogOut } from 'lucide-react';
...
const ROLES = ['Super Admin', 'Administrator', 'Provider Manager', 'Support', 'Sales', 'Content Editor', 'Analyst'];
    ...
    <div className="flex min-h-screen bg-muted/30">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-60 border-r border-border bg-card transition-transform lg:static lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="px-3 py-3 border-b border-border">
          <label className="text-xs font-medium text-muted-foreground">Role (demo)</label>
          <select value={role} onChange={e => setRole(e.target.value)} className="mt-1 w-full rounded-lg border border-border bg-background px-2 py-1.5 text-xs">
      ...
      <div className="flex-1 min-w-0">
        <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-border bg-card px-4">
          ...
          <span className="inline-flex items-center gap-1.5 rounded-full border border-brand/20 bg-brand/10 px-2.5 py-1 text-xs font-semibold text-brand"><ShieldCheck className="h-3.5 w-3.5" /> Admin · {role}</span>
          <p className="ml-auto text-xs text-muted-foreground">Frontend access-control demo. RBAC enforced by backend in production.</p>
        </header>
